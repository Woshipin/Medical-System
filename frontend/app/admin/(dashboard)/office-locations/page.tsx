...
export default function OfficeLocationsPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      showToast("error", "Failed to load clinic locations.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const filteredLocations = useMemo(() => {
    return locations.filter((loc) => {
      ...
      const matchStatus = statusFilter === "all" || loc.status?.toString() === statusFilter;
  ...
  const openEditModal = (loc: OfficeLocation) => {
    setModalMode("edit");
    setErrors({});
    setFormData({ id: loc.id, name: loc.name, status: loc.status ?? 1 });
    setIsModalOpen(true);
  };
            ...
            <table className="w-full whitespace-nowrap text-left border-collapse min-w-[600px]">
              ...
              <tbody className="divide-y divide-slate-100">
                {paginatedLocations.length === 0 ? (
                  <tr><td colSpan={3} className="p-8 text-center text-slate-600 font-medium text-sm">No locations found.</td></tr>
                ) : (
                  paginatedLocations.map((loc) => (
                    <tr key={loc.id} className="hover:bg-slate-50 transition-colors">
                      <td className="px-5 py-3 font-semibold text-slate-900 text-sm">{loc.name}</td>
                      <td className="px-5 py-3">
                        <Badge variant={loc.status === 1 ? "success" : "danger"}>
                          {loc.status === 1 ? "Active" : "Inactive"}
                        </Badge>
                      </td>
                      <td className="px-5 py-3 text-right">
                        <div className="flex justify-end gap-1">
                          <button onClick={() => { setViewData(loc); setIsViewModalOpen(true); }} className="p-1.5 text-slate-500 hover:text-blue-600 hover:bg-blue-50 rounded transition-colors"><Eye className="w-4 h-4" /></button>
                          <button onClick={() => openEditModal(loc)} className="p-1.5 text-slate-500 hover:text-emerald-600 hover:bg-emerald-50 rounded transition-colors"><Edit className="w-4 h-4" /></button>
                          <button onClick={() => { setLocationToDelete(loc.id!); setIsDeleteAlertOpen(true); }} className="p-1.5 text-slate-500 hover:text-red-600 hover:bg-red-50 rounded transition-colors"><Trash2 className="w-4 h-4" /></button>
                        </div>
                      </td>
                    </tr>
                  ))
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg border border-slate-200 overflow-hidden animate-in fade-in zoom-in-95 duration-200 my-auto">
            <div className="flex justify-between items-center px-6 py-4 border-b border-slate-100">
              <h2 className="text-lg font-bold text-slate-900">{modalMode === "create" ? "Create Location" : "Edit Location"}</h2>
            ...
            <div className="p-6 space-y-5">
              <div>
                <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Location Name / Consultation Room</label>
                <div className="relative">
                  <div className={`absolute left-3 top-1/2 -translate-y-1/2 ${errors.name ? 'text-red-400' : 'text-slate-400'}`}><MapPin className="w-4 h-4" /></div>
                  <input type="text" name="name" value={formData.name || ""} onChange={handleInputChange} className={`w-full pl-10 pr-3 py-2.5 text-sm rounded-lg outline-none transition-colors border ${errors.name ? "bg-red-50 border-red-300 text-red-500 placeholder-red-300 focus:ring-2 focus:ring-red-200" : "bg-white border-slate-300 text-slate-900 placeholder-slate-400 focus:ring-2 focus:ring-emerald-500"}`} placeholder="e.g. Consultation Room 101" />
                </div>
            ...
            <div className="px-6 py-4 bg-slate-50 border-t border-slate-200 flex flex-col sm:flex-row justify-end gap-3">
              ...
              <button onClick={handleSave} className="w-full sm:w-auto px-5 py-2.5 text-sm text-white bg-emerald-600 hover:bg-emerald-700 rounded-lg font-semibold shadow-sm transition">Save Location</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md border border-slate-200 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-6 bg-slate-50">
              <div className="grid grid-cols-1 gap-4">
                {[
                  { label: "Location / Room Name", value: viewData.name, icon: MapPin },
      ...
      {isDeleteAlertOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-sm p-6 text-center border border-slate-200 animate-in fade-in zoom-in-95 duration-200">
            ...
            <p className="text-slate-600 text-sm font-medium mb-6">Are you sure you want to delete this location? This action cannot be undone.</p>
