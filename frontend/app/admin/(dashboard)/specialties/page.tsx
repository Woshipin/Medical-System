...
export default function SpecialtiesPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload: Specialty = { name: formData.name, status: targetStatusValue };
      ...
      if (isSuccess) {
        ...
        showToast("success", getBackendMessage(result) || (modalMode === "create" ? "Specialty created successfully!" : "Specialty updated successfully!"));
        fetchData();
      } else {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (res.ok) {
        showToast("success", getBackendMessage(result) || "Specialty deleted successfully.");
        fetchData();
      } else {
        showToast("error", getBackendMessage(result) || "Failed to delete specialty.");
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="space-y-5 max-w-[1400px] xl:max-w-full xl:px-4 2xl:px-6 mx-auto pb-10 px-4 sm:px-6 lg:px-8 relative font-sans antialiased text-slate-900">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pt-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900 sm:text-3xl">Medical Specialties</h1>
          <p className="text-slate-500 mt-1 text-sm font-medium">Manage clinical departments and doctor professional specialties.</p>
        </div>
        <button onClick={openCreateModal} className="w-full sm:w-auto bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2.5 rounded-lg text-sm font-semibold shadow-md transition-all flex items-center justify-center gap-2">
          <Plus className="w-4.5 h-4.5" /> Add Specialty
        </button>
      </div>
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex flex-col lg:flex-row gap-4 items-center justify-between">
        <div className="relative w-full lg:w-[50%]">
          ...
          <input type="text" className="block w-full pl-10 pr-4 py-2.5 border border-slate-300 rounded-lg bg-slate-50 text-slate-900 text-sm placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 font-medium transition-colors" placeholder="Search by specialty name..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden flex flex-col">
        ...
        {!isLoading && filteredSpecialties.length > 0 && (
          <Pagination currentPage={currentPage} totalPages={totalPages} totalItems={filteredSpecialties.length} itemsPerPage={itemsPerPage} onPageChange={setCurrentPage} />
        )}
      </div>
